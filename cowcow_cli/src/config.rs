...
impl Credentials {
    pub fn load(config: &Config) -> Result<Option<Self>> {
        ...
        if creds_path.exists() {
            let content = fs::read_to_string(&creds_path)
                .with_context(|| format!("Failed to read credentials file: {}", creds_path.display()))?;
            
            let creds: Credentials = serde_json::from_str(&content)
                .with_context(|| format!("Failed to parse credentials file: {}", creds_path.display()))?;
    ...
    pub fn save(&self, config: &Config) -> Result<()> {
        ...
        fs::write(&creds_path, content)
            .with_context(|| format!("Failed to write credentials file: {}", creds_path.display()))?;
    ...
    pub fn clear(config: &Config) -> Result<()> {
        ...
        if creds_path.exists() {
            fs::remove_file(&creds_path)
                .with_context(|| format!("Failed to remove credentials file: {}", creds_path.display()))?;
