...
use config::Config;
use auth::{AuthClient, prompt_for_credentials, prompt_for_registration};
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Record { lang, duration, prompt } => {
    ...
    loop {
        ...
        match timeout_result {
            Ok(Some(samples)) => {
                ...
                let actual_duration = Duration::from_secs_f64(total_samples_processed as f64 / samples_per_second as f64);
                ...
                let has_voice_activity = chunk_metrics.vad_ratio > vad_threshold || rms > rms_threshold;
                ...
                if has_voice_activity {
                    ...
                    if silence_start_samples.is_none() {
                        ...
                        silence_start_samples = Some(total_samples_processed - samples.len() as u64);
                    }
                }
                ...
                if let Some(silence_start) = silence_start_samples {
                    ...
                    let silence_duration_secs = silence_duration_samples as f64 / samples_per_second as f64;
                    
                    if silence_duration_secs >= silence_threshold_secs {
                        stop_reason = Some(format!("Silence detected for {:.1}s", silence_duration_secs));
                    }
                }
                ...
                if stop_reason.is_none() {
                    if let Some(dur) = duration {
                        if actual_duration >= dur {
                            stop_reason = Some(format!("Duration reached: {:.2?} (actual audio duration)", actual_duration));
                        }
                    }
                }
...
async fn upload_recordings(force: bool, db: &SqlitePool, config: &Config) -> Result<()> {
    ...
    upload_client.upload_pending_recordings(db, &credentials, force).await?;
...
async fn check_health(config: &Config) -> Result<()> {
    ...
    println!("  Audio device: {}", if device.is_some() { "✅" } else { "❌" });
